...
def get_cells(im, column):
    ...
    _, table_image = cv2.threshold(table_image, 128, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    cv2.imshow("Table", table_image)

    contours, _ = cv2.findContours(table_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

    boundingBoxes = [cv2.boundingRect(c) for c in contours]
    contours, _ = zip(*sorted(zip(contours, boundingBoxes),
            key=lambda k:k[1][1], reverse=False))
        ...
        for cnt in contours:
            ...
            if w > 80 and h > 20:
                ...
                if(i % column == 0):
                    print("<<<< New Row >>>>")
                cell = im[y:y+h, x:x+w]
                print(image_to_string(cell))
                cv2.imwrite("~/Desktop/img{}.png".format(i), cell)
    except:
        pass
    cv2.waitKey()

if __name__ == "__main__":
    im = cv2.imread("~/Desktop/demo.png")
    get_cells(im, 3)